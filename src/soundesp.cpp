...
void c_sound_esp::draw_circle(Color colors, Vector position) noexcept {
	...
	beam_info.m_nModelIndex = interfaces::mdl_info->GetModelIndex("sprites/physbeam.vmt");
	...
	Beam_t* beam = interfaces::view_render_beams->CreateBeamRingPoint(beam_info);
	if (beam)
...
void c_sound_esp::event_player_footstep(IGameEvent* event) noexcept {
	...
	auto walker = reinterpret_cast<c_base_player*>(interfaces::entity_list->GetClientEntity(interfaces::engine_client->GetPlayerForUserID(event->GetInt("userid"))));
...
void c_sound_esp::event_player_hurt(IGameEvent* event) noexcept {
	...
	auto attacker = interfaces::entity_list->GetClientEntity(interfaces::engine_client->GetPlayerForUserID(event->GetInt("attacker")));

	if (!attacker)
		return;

	auto victim = reinterpret_cast<c_base_player*>(interfaces::entity_list->GetClientEntity(interfaces::engine_client->GetPlayerForUserID(event->GetInt("userid"))));
	...
	if (attacker == local_player) {
		if (timer < 1) {
			sound_logs.push_back(c_sound_info(victim->GetAbsOrigin(), interfaces::global_vars->curtime, event->GetInt("userid"))); //victim->GetAbsOrigin()
		}
	}
}
